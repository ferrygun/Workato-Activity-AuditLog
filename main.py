...
def getSession(x_csrf_token, _workato_app_session):
    ...
    body = '{"user":{"email":"WORKATO_USERID","password":"WORKATO_PASSWORD"}}'
...
def getAuditLog(xsrf_token, workato_app_session):
    count = 0
    headers = {
            'Host': ' app.workato.com',
            'User-Agent': ' Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:105.0) Gecko/20100101 Firefox/105.0',
    ...
    datenow = str(tm[0]) + "-" + str('%0.2d' % tm[1]) + "-" + str('%0.2d' % tm[2]) + "T" + str('%0.2d' % tm[3]) + ":" + str('%0.2d' % tm[4]) + ":" + str('%0.2d' % tm[5]) + str('+08:00')
    today = quote(datenow)
    
    datebefore = time.mktime((tm[0], tm[1], tm[2], tm[3]-1, tm[4], tm[5], tm[6], tm[7]))
    tmf = time.localtime(datebefore)
    datebefore = str(tmf[0]) + "-" + str('%0.2d' % tmf[1]) + "-" + str('%0.2d' % tmf[2]) + "T" + str('%0.2d' % tmf[3]) + ":" + str('%0.2d' % tmf[4]) + ":" + str('%0.2d' % tmf[5]) + str('+08:00')
    yesterday = quote(datebefore)

    urllink = 'https://app.workato.com/web_api/activity_logs?created_at_from=' + yesterday + '&created_at_to=' + today + '&user_id=' + USERID_TO_MONITOR
    print(urllink)
    response = urequests.get(urllink, headers=headers, stream=True)
    #print(response.json())
    result = response.json()
    
    for item  in result["result"]["logs"]:
        #print(item["user"]["name"])
        #print(item["event"])

        # skip the Workato service account
        if item["user"]["name"] != 'WORKATO_SVC_ACCOUNT':
            count += 1
...
def main():
    ...
    gc.collect()
    cx = getAuditLog(x_csrf_token, _workato_app_session)
    print(cx)
